...
const Stats: React.FC<{
  ...
  useEffect(() => {
    axios.get(`/users/${userId}/goal`).then((res) => {
  ...
  if (range === "month") {
    ...
    filteredSessions.forEach((session) => {
      ...
      sessionsByWeek[week] =
        (sessionsByWeek[week] || 0) + session.duration / 60;
    });
    ...
    filteredSessions.forEach((session) => {
      ...
      sessionsByMonth[month] =
        (sessionsByMonth[month] || 0) + session.duration / 60;
    });
    ...
    filteredSessions.forEach((session) => {
      ...
      if (segmentStart < segmentEnd) {
        ...
        sessionsByDay[dateLabel] =
          (sessionsByDay[dateLabel] || 0) + minutes / 60;
      }
    });
  ...
  filteredSessions.forEach((session) => {
    ...
    if (sessionMinutes > 0) {
      ...
      perGameStats[gameId].totalMinutes += sessionMinutes;
  ...
  const displayedChartData =
    chartUnit === "hours" ? chartData : chartData.map((h) => h * 60);
        ...
        <div style={{ maxWidth: 600, margin: "2em 0" }}>
          ...
          <div style={{ textAlign: "center", margin: "1em 0" }}>
            ...
            <button
              ...
              style={{
                fontWeight: chartUnit === "minutes" ? "bold" : "normal",
              }}
            >
              Minutes
            </button>
          ...
          <Line
            data={{
              ...
              datasets: [
                {
                  label:
                    chartUnit === "hours" ? "Hours Played" : "Minutes Played",
